...
- 回家后解锁角色互动
- 真实角色立绘显示
- 生成图片保存到 Backend 媒体目录
- 主立绘与动作立绘切换
- 动作卡系统：待机、微笑、打招呼、喝茶、看书、困倦
- 模块化剧情 pack，方便添加 DLC
...
角色进入出租屋后，房间页会加载对应的主立绘和动作素材。点击“生成主立绘”后，Backend 会保存图片；点击“生成动作包”后，可以在普通房间或面对面模式中用动作卡切换角色姿态。未配置图片 API 时，Backend 会生成开发模式占位图，仍可测试完整流程。
